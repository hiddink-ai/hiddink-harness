...
|------------|----------------------|------|
| Source code | `.kimi/skills/` | 재사용 가능한 지식과 워크플로우 정의 |
| Build artifacts | `.kimi/agents/` | 스킬을 조합한 실행 가능한 전문가 |
| Compiler | `mgr-sauron` (R017) | 구조 검증 및 정합성 보장 |
| Spec | `.kimi/rules/` | 빌드 규칙과 제약 조건 |
...
Kimi의 Agent Teams 기능이 활성화되어 있으면 (`CLAUDE_CODE_EXPERIMENTAL_AGENT_TEAMS=1`), 적격한 작업에 적극적으로 사용합니다.
...
| superpowers-developing-for-claude-code | superpowers-marketplace | Kimi 개발 문서 |
