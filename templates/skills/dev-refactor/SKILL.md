...
1. **Load spec**: Read `.claude/specs/<agent-name>.spec.md` (generated by `/hiddink-harness-takeover`)
   - If spec doesn't exist, run takeover first: `/hiddink-harness-takeover <name>`
...
4. **Verify invariants**: After refactoring, check each invariant still holds:
   ...
   ```
5. **Regenerate spec**: If refactoring changed the contract, run `/hiddink-harness-takeover <name>` to update
